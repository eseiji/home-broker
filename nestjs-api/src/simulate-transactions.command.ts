...
@Command({ name: 'simulate-transactions' })
export class SimulateTransactionsCommand extends CommandRunner {
  assets: {
    _id: string | null;
    name: string;
    symbol: string;
  ...
  async createWalletAssets() {
    await this.walletsService.createWalletAsset(this.wallet1._id, {
      assetId: this.assets[0]._id!,
      shares: 10000,
    });
    this.logger.log('Wallet 1 assets created');

    await this.walletsService.createWalletAsset(this.wallet2._id, {
  ...
  async createOrders(generateOrdersClosed: boolean) {
    ...
    for (const index of range(1, 50)) {
      ...
      this.logger.log(
        `Order SELL created: index ${index} - price ${price}`,
      );
